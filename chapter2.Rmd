...
a^x2+bx+c=0. The quadratic formula gives us the solutions:
...
```
n the background, R stores these levels as integers and keeps a map to keep track of the labels. This is more memory efficient than storing all the characters.
...
p<-which.min(murders$population)
states[p]
```
...
```
with over 37 million inhabitants. It is therefore unfair to compare the totals if we are interested in learning how safe the state is. What we really should be computing is the murders per capita. The reports we describe in the motivating section used murders per 100,000 as the unit. To compute this quantity, the powerful vector arithmetic capabilities of R come in 
...
```{r}
murder_rate<-murder_rate <- murders$total / murders$population * 100000 
low<-murder_rate<1
```
...
library(dslabs)
data(murders)
population_in_millions_log<-log10(murders$population)
